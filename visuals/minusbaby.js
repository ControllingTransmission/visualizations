var MinusBaby = function(){
    ...
    init: function(){
      ...
      for(var xpos = 0; xpos < 12; xpos++ ){
        geometry = new THREE.CubeGeometry( window.innerWidth/12, window.innerHeight, window.innerWidth/12 );
    		material = new THREE.MeshBasicMaterial( { color: BARS.colorset[0], wireframe: false, wireframeLinewidth: 10, overdraw: true } );
  ...
  this.ZFIGHTERS = {
    ...
    init: function(){
      ...
      var plane = new THREE.Mesh( new THREE.PlaneGeometry( planeW*50, planeH*50, planeW, planeH ), new   THREE.MeshBasicMaterial( { color: COLORSETS.MINUSBABY[0], wireframe: true, wireframeLinewidth: 10, overdraw: true } ) );
      scene.add(plane);
